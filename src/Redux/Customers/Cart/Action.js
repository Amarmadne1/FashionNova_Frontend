...
export const emptyCart = (jwtToken) => async (dispatch) => {
  ...
  try {
    const response = await fetch(`${API_BASE_URL}/api/cart_items/emptycart`, {
      method: "DELETE",
    ...
    if (response.status === 202) {
      ...
      dispatch({ type: EMPTY_CART_FAILURE, error: "Failed to empty the cart" });
    }
  } catch (error) {
    dispatch({
      type: EMPTY_CART_FAILURE,
      error: "Error occurred while emptying the cart",
    });
  }
};
